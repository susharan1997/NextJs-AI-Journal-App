  ...
  try {
    ...
    if (!userId || !params.id) {
      return NextResponse.json(
        {
          error: `Invalid or missing user Id: ${userId} or journal Id: ${params.id}`,
        },
        { status: 400 }
      );
    }

    const entryAnalysis = await EntryAnalysisModel.findOne({
      entryId: params.id,
    })
      .populate("entryId")
      .exec();
    ...
    if (!entryAnalysis) {
      return NextResponse.json(
        {
          error: `Invalid journal entry data: ${entryAnalysis} ${userId} ${params.id}`,
        },
        { status: 400 }
      );
    }
    ...
    return NextResponse.json(
      { error: `Internal server error: ${error} JOURNAL ENTRY` },
      { status: 500 }
    );
  }
}
...
export async function PATCH(
  ...
  try {
    ...
    const updatedEmbeddings = await openAiEmbeddings.embedQuery(
      summarizedContent
    );
    ...
    return NextResponse.json(
      { error: `Internal server error: ${error} journalId-> ${params.id}` },
      { status: 500 }
    );
  }
}
...
export async function DELETE(
  ...
  try {
    ...
    if (!userId || !journalId) {
      return NextResponse.json(
        { error: `Invalid userId: ${userId} or journalId: ${journalId}` },
        { status: 400 }
      );
    }
    ...
    if (!deleteEntry) {
      return NextResponse.json(
        { error: `Cannot find the journal entry to delete: ${deleteEntry}` },
        { status: 404 }
      );
    }
